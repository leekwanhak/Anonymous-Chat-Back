...
//기본호출주소: http://localhost:5005/api/member

var express = require("express");
var router = express.Router();

//사용자 암호 단방향 암호화 적용을 위해 bcryptjs 참조하기
...
- 호출주소: http://localhost:5005/api/member/entry
- 호출방식: Post
...
router.post("/entry", async (req, res) => {
  ...
  try {
    ...
    //위에 등록할 데이터가 db MEMBER테이블에 저장된후 실제 저장된 회원 정보 다시 반환된다.
...
- 호출주소: http://localhost:5005/api/member/login
- 호출방식: Post
...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    if (member) {
      //Step3: 사용자 암호값 일치여부를 체크- 동일 메일주소가 존재하는 경우
      //사용자 암호 단방향 암호화 적용을 위해 bcryptjs 참조하기-var encrypt = require("bcryptjs"); 위에서 참조하고 객체생성했음
      //함수정의: compare(사용자입력암호, DB에저장된암호,콜백함수(선택)) => 일치하면 true, 아니면 false반환(Promise객체로 결과값을 반환함.)
      //콜백함수: error(에러객체 or null),result(비교결과값(true, false -boolean형임)) => 에러가 발생하면 에러객체가 전달되고 정상처리시 비교결과값이 전달됨.
      //Promise 객체는 비동기 작업의 완료 또는 실패를 나타내는 자바스크립트 객체입니다. Promise 객체는 비동기 작업이 완료되었을 때 결과 값을 반환하거나, 작업이 실패했을 때 오류를 반환합니다.
      //Promise 객체는 세 가지 상태를 가질 수 있습니다: -> try-catch문을 사용하여 비동기 작업을 처리할 때 Promise 객체를 사용합니다.
      // 1) 대기(pending): 초기 상태, 비동기 작업이 아직 완료되지 않은 상태. -> await 키워드를 사용하면 Promise 객체가 이행될 때까지 코드 실행을 일시 중지할 수 있습니다.
      // 2) 이행(fulfilled): 비동기 작업이 성공적으로 완료된 상태, 결과 값이 반환됨. -> Promise 객체가 이행되면 true, flase 반환
      // 3) 거부(rejected): 비동기 작업이 실패한 상태, 오류가 반환됨. -> try-catch문을 사용하여 오류처리
      // Promise 객체를 사용하면 비동기 작업을 보다 간결하고 직관적으로 처리할 수 있습니다. await 키워드를 사용하면 Promise 객체가 이행될 때까지 코드 실행을 일시 중지할 수 있습니다.
      ...
      if (compareResult) {
        // const query = `UPDATE users SET is_online = TRUE, last_login = NOW() WHERE id = ?;`;
        //암호가 일치하는경우
        ...
        apiResult.data = {
          ...
          member: {
            ...
            gender: tokenJsonData.gender,
            department: tokenJsonData.department,
          },
        };
        apiResult.msg = "Ok";
      } else {
...
- 호출주소: http://localhost:5005/api/member/profile
- 호출방식: Get
...
router.get("/profile", async (req, res) => {
  ...
  try {
    ...
    //위에 /login 할 때 var token 변수를 선언해서 사용했었지만 다시 사용할 수 있다
    //왜? var, let, const 이해
    //var: 함수스코프를 가지는 변수선언, 함수내에서 선언된 변수는 함수내에서만 사용가능
    //let: 블록스코프를 가지는 변수선언, 블록({})내에서 선언된 변수는 블록내에서만 사용가능
    //const: 블록스코프를 가지는 변수선언, 블록내에서 선언된 변수는 블록내에서만 사용가능, 상수값을 선언할때 사용
...
// - 호출주소: http://localhost:5005/api/member/profile/upload
// - 호출방식: Post
